...
function GamePage() {
    ...
    const initialBoardData: BoardCellDataType[][] = Array.from({ length: 9 }, (_, row) =>
        Array.from({ length: 9 }, (_, col) =>
            row === 4 && col === 4
                ? { own: "Center", visited: false, blocked: true, territory: "" }
    ...
    const [tempBoardData, setTempBoardData] = useState<BoardCellDataType[][]>(deepCopy(initialBoardData));

    const pos = useRef<Pos>({ x: -1, y: -1 });

    const checkBoardData = useRef<BoardCellDataType[][]>(deepCopy(initialBoardData));
    ...
    const checkTerritory = (x: number, y: number, startOwn: CastleColor): boolean => {
        // 방문기록 남기기
        ...
        if (
            ...
            if (x + 1 === 9) meetWall.current.Right = true;
                ...
                if (checkBoardData.current[y][x + 1].own !== "Center" && checkBoardData.current[y][x + 1].own !== "") {
                    checkBoardData.current[y][x].visited = false;
                    return false;
                }
            }
        ...
        if (
            ...
            if (y + 1 === 9) meetWall.current.Bottom = true;
                ...
                if (checkBoardData.current[y + 1][x].own !== "Center" && checkBoardData.current[y + 1][x].own !== "") {
                    checkBoardData.current[y][x].visited = false;
                    return false;
                }
            }
    ...
    const sendNewPosition = () => {
        ...
        setIsMyTurn(false);
        boardData.current[pos.current?.y][pos.current?.x].blocked = true;
        checkBoardData.current[pos.current?.y][pos.current?.x].blocked = true;
                    ...
                    if (
                        ...
                        if (
                            ...
                            for (let k = 0; k < visitedCells.current.length; k++) {
                                ...
                                boardData.current[el.y][el.x].territory = checkBoardData.current[i][j].own;
                            }
                        }
                    // 아래
                    // 테스트값 초기화
                    ...
                    if (
                        ...
                        if (
                            ...
                            for (let k = 0; k < visitedCells.current.length; k++) {
                                const el = visitedCells.current[k];
                                checkBoardData.current[el.y][el.x].blocked = true;
                                checkBoardData.current[el.y][el.x].territory = checkBoardData.current[i][j].own;
                            }
                        }
                    // 왼
                    // 테스트값 초기화
    ...
    const updateBoard = async (x: number, y: number, own: string, gameState: number) => {
        ...
        // 상대가 배치한 내용 업데이트
            ...
            if (x === -1 || y === -1) {
                ...
                boardData.current[y][x].blocked = true;
                setCheckBoardData();
                setTempBoardData(boardData.current);
            }
    ...
    const setupStomp = async () => {
        try {
            ...
            stomp.subscribe("/topic/gameboard/" + roomId.current, (newMessage: any) => {
                updateBoard(newMessage.x, newMessage.y, newMessage.name, newMessage.gameState);
            });
